...
- **2026-06-14 아날로그 등재타임라인 교차적응증 오염 (바벤시오 등 7종)**: `enrich_amjilsim_join()` 이 `brand_name LIKE 'brand_kr%'` 만으로 `amjilsim_drugs` 의 암질심일을 analog 평가행에 붙임 — 날짜 정합성 부재. `amjilsim_drugs` 는 약제(성분)당 1행(최신 암질심)이라, **신규/확대 적응증의 미래 암질심**이 **과거 적응증의 약평위 평가행**에 잘못 결합. 예: 바벤시오 2020-06-11 약평위(Merkel cell, 허가 2019-03) ↔ 2026-04-15 요로상피암 확대 암질심(매체보도·미verified) 이 한 타임라인에 섞여 "암질심(2026) 후 약평위(2020)" 라는 시간역전 발생. 영향 7행(암질심>약평위): 775(바벤시오)·786(버제니오)·877(사이람자)·902(소발디)·1274(자카비)·1342(카보메틱스)·1581(하보니). 수정: ① `enrich_amjilsim_join` 에 `session_date` SELECT + `amjilsim_pass_date <= session_date` 가드(암질심은 같은 사이클 약평위보다 앞서야 함, 아니면 skip), ② 기존 7행 정리(date>session 항목 제거). 원칙: **암질심→약평위 순서는 같은 등재 사이클 내에서만 성립. 브랜드명 join 에 반드시 날짜 정합성 가드.**
...
- **2026-06-14 KEB 환율 고시회차 최초↔최종 오선택 + 자동갱신 부재**: `fetch_keb_excel` 이 `tmpPbldDvCd_1`(=최초)를 체크해 36개월 평균환율의 '최초' 고시회차를 가져옴(최종 대비 ~0.1% 차이). KEB 폼 라디오 매핑 반직관적(value=1=최초, value=0=최종)이고 `inputCheck()` 가 제출 시 체크된 라디오값으로 `pbldDvCd` hidden 을 재생성하므로 **라디오 체크가 결정적**. 추가로 환율 캐시를 갱신하는 정기 잡이 없어 마지막 xlsx 가 stale(4월에 멈춤)된 상태로 모든 조정가에 사용됨. 수정: ① `tmpPbldDvCd_2`(최종) 체크, ② 월간 자동갱신 잡 `exchange_rate_refresh_job`(매월 2일 04:00) 신설 — 갱신 후 `recompute_foreign_fx()` 로 기존행도 선형 스케일 재계산(adjusted_price_krw 는 환율에 선형), ③ CLI `--fx-refresh-now`/`--fx-recompute-now`. 원칙: **환율은 KEB '최종' 고시회차만. 환율 캐시는 월간 자동갱신이 단일 수집 소스, 검색은 캐시만 소비.** 세부: `agents/rules/foreign_agent_rules.md` §환율 캐시.
