...
**5번째 축 — 한국 약가 제도(KR-RULE) 정합성**: 함께 제공되는 '한국 약가 제도 KR-RULE 사실 base'
(37 micro-rule + 5개 severe violation 패턴)에 비추어 mechanism 분류·confidence 가 한국 제도와
정합하는지 판단하라. 특히 다음 severe violation 을 점검:
- LOE 도래 자산을 미래시제로 분석 / 단독품목 면제 자산에 generic 인하 적용
- 추가 인하 여력 소진 자산에 추가 인하 분석 / 기체결 RSA 자산 표시가로 mechanism 단정
- 4대 기전 분류가 KR-RULE 산정률(예: 첫 제네릭 70% Year 1, 누적 53.55% Year 1+)과 불일치
가능하면 issues·final_verdict 에 KR-RULE 번호를 인용하라.
...
class ReviewAgent:
    def __init__(self, openai_model: str = "gpt-4o", gemini_model: str = "gemini-2.5-flash"):
    ...
    def review_price_change_reason(
        ...
        # ── 한국 약가 제도 KR-RULE 사실 base 주입 (korea-drug-pricing-system 스킬) ──
        #   결과 판단을 37 micro-rule + 5 severe violation 에 비추어 수행. 두 리뷰어 공유.
        try:
            from agents.market_intelligence.skill_kb import build_kr_rule_context
            kr_ctx = build_kr_rule_context(
                drug_name=original_request.get("drug", ""),
                reason_hint=str(original_request.get("delta_pct", "")),
            )
        except Exception as e:
            logger.warning("[ReviewAgent] KR-RULE 컨텍스트 로드 실패: %s", e)
            kr_ctx = ""
        rules_with_kb = mi_rules + (("\n\n" + kr_ctx) if kr_ctx else "")

        # ── 2단계: LLM 패널 (OpenAI + Gemini 독립 리뷰) ─────────────────
        openai_verdict = self._llm_review(original_request, mi_result, rules_with_kb)
        openai_verdict.setdefault("reviewer", "openai-" + self.model)

        gemini_verdict = self.gemini.review(original_request, mi_result, rules_with_kb) \
    ...
    def _merge_panel(self, openai_v: dict, gemini_v, mechanical: dict) -> dict:
        ...
        # 합의 규칙:
        #   - 패널 전원 approved → approve (full consensus)
        #   - 과반수 approved      → approve (split — 의견 차이는 notes 에 기록)
        #   - 패널 전원 reject     → reject
        #   기계적 blocker 는 별도로 has_blocker 에서 이미 reject 처리됨.
        n = len(approvals) or 1
        yes = sum(approvals)
        # 기계적 blocker 는 이미 상위 단계에서 거부됨 (여기 도달 = 하드 규칙 통과).
        # LLM 패널 합의 규칙:
        #   - 전원 승인 → 완전 합의 (full consensus)
        #   - 일부 승인 → split 승인 (거짓 양성 방지, 신뢰도 하향)
        #   - 전원 거부 → 거부
        approved_final = yes >= 1
        split = 0 < yes < n
        ...
        if yes == n and n >= 2:
            reason = "패널 전원 합의 승인"
        elif approved_final and split:
            reason = f"패널 과반수 승인 ({yes}/{n}) — 의견 엇갈림"
        elif approved_final:
            reason = "리뷰 승인"
        else:
            reason = "패널 합의 거부"
        # 의견 엇갈릴 때는 score 상한 하향 (신뢰도 조절)
        if split:
            score = min(score, 75)
        ...
        return {
            ...
            "review_mode": ("panel-split" if split else "panel") if gemini_v else "openai-only",
            "panel": panel,
            "panel_size": len(panel),
            "consensus": {"yes": yes, "total": n, "split": split},
        }
    ...
    def _mechanical_check(self, req: dict, result: dict) -> dict:
        ...
        if dt:
            ...
            mentioned_years = set(int(y) for y in re.findall(r"(?<!\d)(19\d{2}|20\d{2})(?!\d)", reason))
    ...
    def _llm_review(self, req: dict, result: dict, mi_rules: str) -> dict:
        try:
            ...
            user_msg = (
                ...
                f"[프로젝트 룰 + 한국 약가 제도 KR-RULE 사실 base]\n{mi_rules}\n\n"
                f"[ReviewAgent 룰 원문]\n{self.review_rules}\n\n"
                f"위 4가지 축 + 5번째 축(KR-RULE 정합성)을 점검하고 JSON으로만 응답하세요."
            )
