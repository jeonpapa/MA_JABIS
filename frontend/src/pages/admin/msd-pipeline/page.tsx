...
import {
  listPipeline, createPipeline, updatePipeline, deletePipeline, parseNoteExtras,
...
export default function AdminMsdPipelinePage() {
  ...
  // 주의: note 는 Home PipelineModal 의 custom JSON 일 수 있으므로 trim 만 하고 원문 그대로 round-trip.
                  ...
                  {items.map(it => (
                    editId === it.id ? (
                      ...
                      <tr key={it.id} className="border-b border-[#1E2530]/50 last:border-b-0 hover:bg-[#1E2530]/30">
                        ...
                        <td className="py-2 pr-3 text-[#8B9BB4] text-xs max-w-[220px]"><NoteCell note={it.note} /></td>
                        <td className="py-2 text-right whitespace-nowrap">
...
/**
 * note 셀 렌더 — Home PipelineModal 이 저장한 custom JSON(`{"custom":true,...}`) 은
 * raw JSON 대신 배지 + 부가 필드 요약으로 표시한다. 일반 텍스트 메모는 그대로 표시.
 * 편집 시에는 raw 문자열이 그대로 round-trip 되므로 JSON 이 깨지지 않는다.
 */
function NoteCell({ note }: { note: string | null }) {
  if (!note) return <span className="text-[#4A5568]">—</span>;
  const extras = parseNoteExtras(note);
  if (extras && extras.custom === true) {
    const parts = [
      extras.drugClass,
      extras.targetDisease,
      extras.domesticApprovalDate ? `허가 ${extras.domesticApprovalDate}` : undefined,
      extras.domesticReimbursementDate ? `급여 ${extras.domesticReimbursementDate}` : undefined,
    ].filter(Boolean);
    return (
      <span className="inline-flex items-center gap-1.5 max-w-full">
        <span className="text-[9px] px-1.5 py-0.5 rounded-full font-medium bg-[#7C3AED]/20 text-[#7C3AED] flex-shrink-0">custom</span>
        <span className="truncate">{parts.length > 0 ? parts.join(' · ') : 'Home 등록 항목'}</span>
      </span>
    );
  }
  return <span className="block truncate">{note}</span>;
}
